...
impl Config {
    pub fn new(shell: MultiShell) -> TurboResult<Config> {
        let  cfg = Config {
        /*    home_path: try!(homedir().chain_error(|| {
                human("Meg couldn't find your home directory. \
                      This probably means that $HOME was not set.")
            })),
            meg_email: String::new(),
            meg_apikey: String::new(),
...
/*fn homedir() -> Option<PathBuf> {
    let megam_home = env::var_os("MEGAM_HOME").map(PathBuf::from);
    let user_home = env::home_dir().map(|p| p.join(".megam"));
    return megam_home.or(user_home);
}
